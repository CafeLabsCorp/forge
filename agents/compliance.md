...
- **Store requirements are concrete checklist items, not vague guidance.**
  ...
  account-deletion path (in-app or web) per current Play policy. These
  requirements shift over time — verify against current store policy via
  WebFetch rather than relying on possibly-stale knowledge.
...
1. **Inventory what the product actually collects and processes** — accounts,
   payment info, location, device data, analytics — sourced from what
...
3. **Produce an LGPD checklist** specific to this inventory: legal basis per
   data point, retention, user rights, and whether a DPO is warranted at this
   scale.
